...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        let groundBody = SKPhysicsBody(rectangleOf: CGSize(width: 1346, height: 1))
        
        // Posicionamento do chão
        ground.position = CGPoint(x: 673, y: -10)
        ...
        beer = SKSpriteNode(imageNamed: "beer")
        ...
        let beerBody = SKPhysicsBody(rectangleOf: CGSize(width: 50, height: 50))
        //          beerBody.affectedByGravity = true
        beerBody.isDynamic = false
        // Atribuímos o corpo físico do presente à representação gráfica
        beer.physicsBody = beerBody
    ...
    func setupWalkAnimation() {
        ...
        walkList.append(SKTexture(imageNamed: "c1"))
        walkList.append(SKTexture(imageNamed: "c2"))
        walkList.append(SKTexture(imageNamed: "c3"))
        walkList.append(SKTexture(imageNamed: "c4"))
        walkList.append(SKTexture(imageNamed: "c5"))
    ...
    func setupJumpAnimation() {
        ...
        let jumpTexture = SKTexture(imageNamed: "j2")
    ...
    func setupLayer(_ layer: Int) {
        ...
        let sprite1 = SKSpriteNode(imageNamed: "space\(layer)")
        let sprite2 = SKSpriteNode(imageNamed: "space\(layer)")
        ...
        layerNode.position = CGPoint(x: self.size.width * 0.5, y: self.size.height * 0.6)
        ...
        let moveLeft = SKAction.moveBy(x: -self.size.width, y: 0, duration: TimeInterval(1 * (10 - layer)))
    ...
    func setupBeerAnimation() {
        // Animações de movimentos do presente para cima e para baixo
        var beerDown = SKAction.moveBy(x: 0, y: -350, duration: 2)
        var beerUp = SKAction.moveBy(x: 0, y: 300, duration: 2)
        
        // Suaviza as animações para que o objeto acelere e desacelere. Fica mais natural
        beerDown.timingMode = .easeInEaseOut
        beerUp.timingMode = .easeInEaseOut
        
        // Cria uma sequência com as animações para cima e para baixo
        var beerDownUp = SKAction.sequence([beerDown, beerUp])
        // Repete a sequência para sempre
        var beerDownUpForever = SKAction.repeatForever(beerDownUp)
        
        // Animação para mover o presente lentamente para a esquerda e de volta para a direita
        var beerMoveLeft = SKAction.moveBy(x: -2100, y: 0, duration: 14)
        var beerMoveRight = SKAction.moveBy(x: 2100, y: 0, duration: 0)
        var beerLeftRight = SKAction.sequence([beerMoveLeft, beerMoveRight])
        
        // Loop infinito para fazer o presente ficar passando eternamente caso o personagem não pegue-o
        var beerLoop = SKAction.repeatForever(beerLeftRight)
        
        // Une a animação para cima e para baixo com a animação para esquerda e direita 
        var beerAnimation = SKAction.group([beerDownUpForever, beerLoop])
        
        // Colocando o presente em um ponto para a direita fora da tela
        beer.position = CGPoint(x: 2000, y: 500)
        
        // Manda o presente rodar a animação completa que foi criada
        beer.run(beerAnimation)
    }
    ...
    func jump() {
        ...
        player.physicsBody!.velocity = CGVector(dx: 0, dy: 800)
    }
    ...
    func getTheBeer() {
        ...
        let label = SKLabelNode(text: "Ausgezeichnet")
        // Definimos a fonte
        ...
        var labelFadeIn = SKAction.fadeIn(withDuration: 2)
